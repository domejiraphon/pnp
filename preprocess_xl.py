...
class Preprocess(nn.Module):
    ...
    class Config:
        ...
        prompt: str = ""
        half_precision_weights: bool = False # to prevent nan value
        seed: int = 1024
    ...
    def ddim_inversion(self, cond, latent, save_path, save_latents=True,
                                timesteps_to_save=None, added_cond_kwargs=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float32):
            for i, t in enumerate(tqdm(timesteps)):
                ...
                neg_alpha = self.scheduler.alphas_cumprod[0] if self.sd_version == "xl" else self.scheduler.final_alpha_cumprod
    ...
    @torch.no_grad()
    def ddim_sample(self, x, cond, save_path, save_latents=False, 
                timesteps_to_save=None, added_cond_kwargs=None):
        # self.scheduler.set_timesteps(self.scheduler.num_inference_steps, 
        #             device=self.device)
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float32):
            for i, t in enumerate(tqdm(timesteps)):
                    ...
                    alpha_prod_t_prev = (
                        self.scheduler.alphas_cumprod[timesteps[i + 1]]
                        if i < len(timesteps) - 1 else neg_alpha
                    )
                    mu = alpha_prod_t ** 0.5
                    sigma = (1 - alpha_prod_t) ** 0.5
                    mu_prev = alpha_prod_t_prev ** 0.5
    ...
    def extract_latents(self, num_steps, data_path, save_path, timesteps_to_save,
        ...
        if self.sd_version == "xl":
            ...
            )  = self.pipe.encode_prompt(inversion_prompt, 
                                device=self.device, 
            ...
            prompt_embeds = self.get_text_embeds(inversion_prompt, "")[1].unsqueeze(0)
        inverted_x = self.inversion_func(prompt_embeds, 
                                        latent, 
                                        save_path, 
                                        save_latents=not extract_reverse,
                                        timesteps_to_save=timesteps_to_save,
                                        added_cond_kwargs=added_cond_kwargs,
                                        )
        
        latent_reconstruction = self.ddim_sample(inverted_x, 
                                                prompt_embeds, 
                                                save_path, 
                                                save_latents=extract_reverse,
                                                timesteps_to_save=timesteps_to_save,
                                                added_cond_kwargs=added_cond_kwargs,
                                                )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sd_version', type=str, default='2.1', choices=['1.5', '2.0', '2.1', 'xl'],
